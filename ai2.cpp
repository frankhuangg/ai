...
bool is_safe(Node* n){	// 判斷兩岸的傳教士是否安全
	...
	if(m_left<c_left && m_left!=0||cost>limit)		// 左岸食人魔數量不可超過傳教士數量，除非傳教士數量為0
		return false;
	if(m_right<c_right && m_right!=0||cost>limit)	// 右岸食人魔數量不可超過傳教士數量，除非傳教士數量為0
		return false;
	if(m_left<0 || c_left<0 || m_right<0 || c_right<0||cost>limit)//limit為船資之上限，cost不可超過limit
		return false;
	if((m_left>=c_left || m_right>=c_right)&&cost<=limit)
		return true;
}
...
void refresh_opened(Node* n){	// 如果節點有較低的分數或步數，更新opened_list
	...
	opened_list.push_back(Node(n->m, n->c, n->a, n->b, n->step, n->cost, n->time, n->parent));
}
...
void a_star_algorithm(){
	...
	while(opened_list.size() != 0){
		...
		// 將取出的點加入closed_list中
		closed_list.push_back(Node(node.m, node.c, node.a, node.b, node.step, node.cost, node.time, node.parent));
		// 判斷取出的點是否為目標點
...
int main(int argc, char const *argv[]){
	...
	Node start_node(m_num, c_num, 1, 1, 0, 0, 0, nullptr);
	...
	// cout << endl;
	output_result();
    cout << endl << "花費的時間：" << totaltime << " S" << endl;
	// m_num = 2;
	// c_num = 0;
